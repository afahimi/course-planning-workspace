...
export function CourseDetailPopup({ isOpen, onClose, course }: CourseDetailProps) {
  const { addCourseToWorklist, removeCourseFromWorklist, currentWorklist } = useAppContext()
  ...
  useEffect(() => {
    if (course) {
      // If the course is in the worklist, pre-select the current section
      const courseIndex = currentWorklist.courses.indexOf(course.id)
      if (courseIndex !== -1) {
        setSelectedSection(currentWorklist.sections[courseIndex])
      } else {
        setSelectedSection(null)
      }

      // Default to sections tab if course is already in schedule
      if (courseIndex !== -1 && activeTab === "about") {
        setActiveTab("sections")
      }
    }
  }, [course, currentWorklist, activeTab])

  if (!course) return null

  const isCourseInWorklist = currentWorklist.courses.includes(course.id)
  const currentSectionId = isCourseInWorklist
    ? currentWorklist.sections[currentWorklist.courses.indexOf(course.id)]
    : null

  const handleAddOrUpdateSection = () => {
    if (!selectedSection) return

    // If course is already in worklist, remove it first (to update the section)
    if (isCourseInWorklist) {
      removeCourseFromWorklist(course.id)
    }

    // Add course with the selected section
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-auto p-4">
            ...
            <TabsContent value="sections" className="mt-0 h-full data-[state=active]:flex data-[state=active]:flex-col">
              ...
              <p className="text-sm text-gray-600 mb-4">
                {isCourseInWorklist
                  ? "You can switch to a different section by selecting it below."
                  : "Select a section to add to your schedule. Each section may have different instructors, times, and locations."}
              </p>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="font-semibold flex items-center gap-2">
                          ...
                          {section.id === currentSectionId && selectedSection !== section.id && (
        ...
        <DialogFooter className="border-t pt-4">
          ...
          <Button disabled={!selectedSection && activeTab === "sections"} onClick={handleAddOrUpdateSection}>
            {isCourseInWorklist
